...
> 本文档涵盖台球陪练平台裁教端（`/coach-api/billiard/**`）的核心接口定义，包含裁教自身管理、订单处理、计时、钱包、财务、申诉等模块。
> 
> **鉴权说明**：所有裁教端接口均需校验 Token，从 `SecurityFrameworkUtils.getLoginUserId()` 取当前裁教的 `system_users.id`，再关联 `billiard_coach.user_id` 确认裁教档案存在且状态正常。
...
- 下线时，若裁教有进行中的订单，不允许切换（由订单模块服务层判断）
...
**功能**：返回当前登录裁教的最小工作状态信息，用于裁教端首页快速刷新上下线状态。
...
### 1.3 POST /coach-api/billiard/coach/location — 更新裁教坐标

**功能**：裁教在登录后主动上报当前位置坐标，供用户端距离排序使用。
...
- 仅允许在线裁教调用；下线状态调用返回业务异常
- 增加 5 秒限频，防止客户端高频上报坐标
- 登录接口在 `system` 模块，保持模块边界不改动；裁教端登录成功后第一步调用该接口
...
**功能**：裁教查看自己的业务档案（艺名、级别、简介、照片、证书等）。

**响应字段**：与裁教详情（App 端）相同，额外返回 `commissionRate`（扣佣比例，仅裁教可见）。

---

### 1.5 POST /coach-api/billiard/coach/free-travel — 切换低碳出行开关

**功能**：裁教主动开启或关闭“低碳出行”。开启后，用户端展示“低碳出行”标签，且用户下单时车费记为 0。
...
- 若开启低碳出行，自动为该裁教的 `tags` 追加“低碳出行”标签；若关闭则移除该标签。

**响应**：`CommonResult<Boolean>`

---

### 1.6 GET /coach-api/billiard/coach/dashboard — 获取裁教看板

**功能**：返回裁教今日的服务时长、服务次数和预计收入（个人中心简易看板）。
...
### 1.7 GET /coach-api/billiard/coach/statistics — 查询当前裁教综合统计

**功能**：查询当前登录裁教在指定时间范围内的上钟总时长、完成订单数、已到账提现总金额。
...
**功能**：返回当前裁教的待接单列表（status=PENDING_ACCEPT）。

**业务过滤：** `coach_id = 当前裁教.billiard_coach.id AND status = 20 AND deleted = 0`
...
| `expireAt`        | long | 是 | 接单截止时间（毫秒时间戳，pay_time + 5分钟），裁教端倒计时    |
...
1. 更新 `status` = CANCELLED，`cancel_time` = 当前时间，`cancel_type` = 2，`cancel_reason` = "裁教拒绝接单"
2. 调用 `PayRefundApi.createRefund(...)`，`merchantRefundId` = `REJECT_{orderId}`，全额退款
3. 推送通知给用户：`"裁教暂时无法接单，已为您全额退款"`
...
**功能**：返回裁教当前进行中的订单完整信息（含脱敏手机号、球厅导航、剩余时长）。
...
**过滤：** `coach_id = 当前裁教 AND status = 40 AND deleted = 0`，最多返回 1 条（PRD 规定一次只服务一位）

---

### 2.9 GET /coach-api/billiard/order/page — 裁教统一订单列表（合并历史与全部）

**功能**：返回裁教的所有订单，前端不再区分历史订单和全部订单页面，统一调用此接口。

**请求参数：**`pageNo`、`pageSize`、`status`（可选，不传返回全部）

**固定过滤：** `coach_id = 当前裁教 AND deleted = 0`，按 `create_time DESC` 排序
...
### 2.10 GET /coach-api/billiard/order/get?id={id} — 裁教订单详情（含评价）

**功能**：裁教查看本人订单详情，返回订单全量信息并聚合对应评价信息。
...
1. 订单必须存在
2. `order.coach_id` 必须等于当前登录裁教对应 `billiard_coach.id`（防越权）
...
### 2.11 GET /coach-api/billiard/coach/review/page — 裁教查看自己所有历史评价

**功能**：裁教分页查看自己收到的全部历史评价，返回结构参考用户端裁教主页评价列表。
...
**业务过滤：** 当前登录系统用户映射到 `billiard_coach.id` 后，按 `coach_id = 当前裁教ID AND deleted = 0` 查询，按评价时间倒序。
...
### 4.1 GET /coach-api/billiard/wallet/balance — 查询裁教钱包余额

**功能**：查询当前登录裁教的钱包 ID、可用余额和冻结金额。
...
### 4.2 GET /coach-api/billiard/wallet/transaction/page — 查询裁教钱包流水分页

**功能**：分页查询当前登录裁教的钱包流水，包含订单收益、平台扣款、客服补偿、提现打款等记录。
...
| `bizTypeCode` | string | 是 | 裁教端业务类型编码，枚举见下方 `bizTypeCode` |
| `bizTypeDesc` | string | 是 | 裁教端业务类型名称 |
| `title` | string | 是 | 流水标题 |
| `price` | int | 是 | 交易金额（分） |
...
**功能**：当前登录裁教提交提现申请，后端会先冻结对应金额，再写入 `billiard_withdrawal`。
...
- 裁教必须存在。
- `withdrawAmount > 0`。
- 同一裁教存在 `PENDING(0)` 或 `PROCESSING(1)` 提现申请时，不允许重复提交。
...
### 5.1 GET /coach-api/billiard/finance/dashboard — 裁教数据看板

**功能**：返回当日汇总数据，用于裁教端工作台首屏展示。
...
| 指标                    | 数据源                                     | 过滤条件                                                                               |
| --------------------- | --------------------------------------- | ---------------------------------------------------------------------------------- |
| `todayDuration`       | `billiard_timer_record.actual_duration` | `coach_id = 当前裁教 AND end_time BETWEEN 今日00:00 AND NOW()`                           |
| `todayServiceCount`   | `billiard_order`                        | `coach_id = 当前裁教 AND status = 60 AND end_time BETWEEN 今日00:00 AND NOW()`           |
| `todayExpectedIncome` | `billiard_finance_settlement`           | `coach_id = 当前裁教 AND settle_time BETWEEN 今日00:00 AND NOW() AND settle_status >= 1` |

**性能建议：**
- 接口建议加 Redis 缓存，key = `coach:dashboard:{coachId}`，TTL = 60 秒
- 裁教切换上下线、订单状态变更时主动清除缓存

---

### 5.2 GET /coach-api/billiard/finance/settlement/page — 裁教收益明细分页

**功能**：裁教查询自身历史结算记录（收益流水）。
...
**固定过滤：** `coach_id = 当前裁教 AND deleted = 0 AND settle_status >= 1`，按 `settle_time DESC` 排序
...
鉴权：裁教端 Token（`system_users` 体系）

### 7.1 POST /coach-api/billiard/exception/report — 裁教报告异常

**功能**：裁教在到达服务地址后，如果联系不到客户，可向负责人反馈异常。

**请求体（CoachExceptionReportReqVO）：**

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `orderId` | bigint | 是 | billiard_order.id，必须归属当前登录裁教 |
...
3. 推送站内通知给客服组：`"新异常订单待处理：订单 ${orderNo}，裁教反馈：${reason}"`
...
### 6.1 POST /coach-api/billiard/appeal/submit — 裁教发起申诉
...
3. 推送通知给客服组：`"裁教 ${stageName} 对异常订单 ${exceptionOrderId} 发起申诉"`

**响应体：**`Long` (返回新创建的 appealId)

---

### 6.2 GET /coach-api/billiard/appeal/page — 裁教查看申诉进度
